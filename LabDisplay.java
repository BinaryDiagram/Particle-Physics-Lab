...
public class LabDisplay extends JComponent implements MouseListener,
   MouseMotionListener, ActionListener, ChangeListener
{
   ...
   public LabDisplay(String title, int numRows, int numCols, String[] buttonNames)
   {
      ...
      //determine cell size
      //cellSize = Math.max(1, 800 / Math.max(numRows, numCols));  //daf 600 to 900
      cellSize = Math.max(1, ParticleLab.getCellSize() / Math.max(numRows, numCols));  //daf 600 to 900
   ...
   {
      return (int)Math.pow(10, 0.03 * sliderValue + 3);  //daf - changed 0.03 to 0.01
   }
}